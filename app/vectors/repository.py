...
class VectorRepo:
    ...
    async def upsert_embedding(self, client_id: str, *, entity_type: str,
                                entity_id: str, content: str, embedding: list[float],
            ...
            async with pool.acquire() as conn:
                row = await conn.fetchrow(
                    ...
                    DO UPDATE SET
                        ...
                        embbeding = EXCLUDED.embedding,
                        metadata = EXCLUDED.metadata,
                        updated_at = now(),
                        deleted_at = NULL
                    RETURNING id, client_id, entity_type, entity_id, content, metadata, created_at, updated_at
                    """, cid, entity_type, eid, content, np.array(embedding, dtype=np.float32), metadata
                )
    ...
    async def search(self, client_id: str, *, query_embed: list[float],
                        top_k: int=5, entity_type: Optional[str] = None) -> list[dict[str, Any]]:
        try:
            cid =  client_id if isinstance(client_id, uuid.UUID) else uuid.UUID(client_id)
            pool = await init_db()
            async with pool.acquire() as conn:
                rows = conn.fetch(
                    ...
                    LIMIT $4
                    """, np.array(query_embed, dtype=np.float32), cid,
                    entity_type, int(top_k)
                )
